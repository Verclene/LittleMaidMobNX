...
前提MOD
    ...
    EBLib ver.0.1.1 以上
    ※このMODは、Forge 11.14.3.1446でビルドされています。
...
注意
    ・1.6.2以前のメイドさんは移行はできません。
    ・1.7のメイドさんも移行できるか分かりません。クラスがほぼ一緒なのでもしかしたら引き継げるかも

サウンドパック導入方法
    デフォルトのサウンドはガストなので、早いうちにメイドさんの声を変えてあげてください。
    サウンドパックは「サウンドがogg形式で構成されているもの」を読み込めます。zipのまま、modsディレクトリに入れてください。
    一度に認識できるサウンドパックは1つのみです。先に認識された方を優先して使用します。
    ※パック内に日本語を含む名前のファイルがある場合は、パックを解凍してください。

マルチモデル導入方法
    zipのままmodsに入れれば読み取ります。野生メイド用テクスチャも使用できます。
    ※装備の表示は未実装です。
    ※野生スポーンのテクスチャを増やしたい場合は、以下のページを参考にしてください。
    http://forum.minecraftuser.jp/viewtopic.php?f=13&t=23347&start=420#p232492
